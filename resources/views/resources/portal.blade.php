...
<body>
  <div class="shell">
    <header class="topbar">
      ...
      <a class="back" href="https://zaoguang-vpn.com/dashboard">Về trang khách hàng</a>
    </header>

    <main>
      <section class="hero">
        <span class="eyebrow">✓ Kho tải chính thức</span>
        <h1>{{ $config['title'] }}</h1>
        <p>{{ $config['subtitle'] }}</p>
        @if($config['notice'])<div class="notice">{{ $config['notice'] }}</div>@endif
      </section>

      <section class="grid" aria-label="Danh sách ứng dụng">
        @forelse($apps as $app)
          <article class="card">
            ...
            <div class="platform">{{ strtoupper($app['platform']) }}</div>
            <h2>{{ $app['name'] }}</h2>
            @if($app['version'])<span class="version">Phiên bản {{ $app['version'] }}</span>@endif
            @if($app['description'])<p class="description">{{ $app['description'] }}</p>@endif
            <a class="download" href="{{ $app['download_url'] }}" target="_blank" rel="noopener noreferrer">Tải xuống <span aria-hidden="true">↓</span></a>
          </article>
        @empty
          <div class="empty">Danh sách ứng dụng đang được cập nhật. Vui lòng quay lại sau.</div>
        @endforelse
      </section>
    </main>

    <footer class="footer">
      <span>© {{ date('Y') }} {{ $appName }}. Liên kết tải xuống chính thức.</span>
      <span>@if($config['support_url'])<a href="{{ $config['support_url'] }}">Cần hỗ trợ?</a> · @endif<a href="/manage">Quản trị kho tải</a></span>
    </footer>
  </div>
</body>
</html>
